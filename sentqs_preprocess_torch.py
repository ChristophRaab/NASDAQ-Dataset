...
def create_datasets(x_train, y_train, x_val, y_val, batch_size=4, shuffle=True):
    class TensorDatasetTransforms(TensorDataset):
        def __init__(self, x, y):
            super().__init__(x,y)
        def __getitem__(self, i):
            #tensor = data_transform(self.tensors[0][i]) # Options to transform the dataset
            tensor = self.tensors[0][i]
            return (tensor,) + tuple(t[i] for t in self.tensors[1:])

    train_dataset = TensorDatasetTransforms(torch.tensor(x_train), torch.tensor(y_train))
    train_loader = DataLoader(train_dataset, batch_size=batch_size,
                                              shuffle=shuffle, num_workers=2)

    val_dataset = TensorDatasetTransforms(torch.tensor(x_val), torch.tensor(y_val))
...
def generate_embedding_model(text, y, batch_size=256, epochs = 100, save = True, dim = 200, val_split=0.2):
    ...
    # Get Model
    model = LinearModel(embedding_matrix_gl=glove, embedding_matrix_sg=skipgram, number_class=3)
